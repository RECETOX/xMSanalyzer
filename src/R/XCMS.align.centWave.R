XCMS.align.centWave <- function(
                                cdfloc,
                                XCMS.outloc,
                                ppm.list = c(10),
                                mz.diff.list = c(-0.001),
                                sn.thresh.list = c(10),
  ...
  for (t in sn.thresh.list)
  {
    for (p in ppm.list)
    {
      for (m in mz.diff.list)
      {
        ...
        for (s in step.list)
        {
          xset <- xcmsSet(cdf_files,
            method = "centWave", ppm = p, snthresh = t, mzdiff = m, peakwidth = peakwidth, prefilter = prefilter.list,
          ...
          for (b in bw.val) {
            ...
            if (is.na(run.order.file) == FALSE) {
              ...
              ordlist <- sapply(1:length(fileorder), function(i) {
            ...
            if (is.na(target.mz.list) == FALSE) {
              ...
              overlapres5ppm <- getVenn(
                dataA = finalfeatmat, name_a = paste("Expdata", sep = ""), dataB = stddata, name_b = "Target", mz.thresh = 10, time.thresh = NA, alignment.tool = "XCMS",
