evaluate.Samples <- function(curdata,
                             numreplicates,
                             alignment.tool,
                             cormethod = "pearson",
                             missingvalue = 0,
                             ignore.missing = TRUE,
                             replace.bad.replicates = TRUE) {
  col_end <- get_num_columns(alignment.tool)
  ...
  for (samp in seq(1, numsamp, numreplicates))
  {
    ...
    if (numreplicates > 2) {
      if (num_bad_reps == 1) {
        ...
        subdata[, c(bad_rep)] <- apply(subdata[, -c(bad_rep)], 1, function(x) {
          mean(x, na.rm = TRUE)
        })
      }
    }
