xMSwrapper.XCMS.centWave <- function(
                                     cdfloc,
                                     XCMS.outloc,
                                     xMSanalyzer.outloc,
                                     ppm.list = c(10, 25, 30),
                                     mz.diff.list = c(-0.001, 0.1),
                                     sn.thresh.list = c(3, 5, 10),
                                     prefilter.list = c(3, 100),
                                     bw.val = c(10, 30),
                                     ...
                                     max.mz.diff = 15,
                                     max.rt.diff = 300,
                                     merge.eval.pvalue = 0.2,
                                     mergecorthresh = 0.7,
                                     deltamzminmax.tol = 10,
                                     num_replicates = 2,
                                     subs = NA,
                                     mz.tolerance.dbmatch = 10,
                                     adduct.list = c("M+H"),
                                     samp.filt.thresh = 0.70,
                                     feat.filt.thresh = 50,
                                     cormethod = "pearson",
                                     mult.test.cor = TRUE,
                                     missingvalue = 0,
  ...
  if (is.na(cdfloc) == FALSE) {
    ...
    if (is.na(XCMS.outloc) == FALSE) {
      ...
      data_rpd_all <- XCMS.align.centWave(
        cdfloc,
        XCMS.outloc,
        ppm.list = ppm.list,
        mz.diff.list = mz.diff.list,
        sn.thresh.list = sn.thresh.list,
        prefilter.list = prefilter.list,
        bw.val = bw.val,
        groupval.method = groupval.method,
        step.list = step.list,
        max = max,
        minfrac.val = minfrac.val,
        minsamp.val = minsamp.val,
        mzwid.val = mzwid.val,
        sleep.val = sleep.val,
        run.order.file,
        subs,
        retcor.method = retcor.method,
        retcor.family = retcor.family,
...

  {
    ...
    if (length(data_rpd_all) > 0) {
        ...
        for (i in 1:length(alignmentresults))
        {
          ...
          for (j in i:length(alignmentresults))
          {
            ...
            if (bool_num == 1) {
              ...
              numpeaks <- lapply(1:dim(data_m_int)[1], function(j) {
                length(which(is.na(data_m_int[j, ]) == FALSE))
              })
      ...
      if (is.na(sample_info_file) == FALSE) {
        ...
        maxint <- apply(adjdata2[, -c(1:8)], 1, function(x) {
          max(x, na.rm = TRUE)
        })
  ...
  return(list(
    "XCMS.merged.res" = union_list, "XCMS.ind.res" = data_rpd_all, "XCMS.ind.res.filtered" = data_rpd, "final.feat.table.annot" = annotres_list, "feat.eval.ind" = feateval_list, "sample.eval.ind" = rsqres_list, "final.feat.table.raw" = d1, "final.feat.table.combat" = adjdata2,
