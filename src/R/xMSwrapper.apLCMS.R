xMSwrapper.apLCMS <- function(
  ...
  if (is.na(cdfloc) == FALSE) {
    ...
    if (is.na(apLCMS.outloc) == FALSE) {
      data_rpd_all <- apLCMS.align(cdfloc, apLCMS.outloc, min.run.list, min.pres.list,
        minexp = minexp, mztol = mztol, alignmztol = alignmztol, alignchrtol = alignchrtol,
  ...
  if (length(alignmentresults) > 0) {
    ...
    if (is.na(sample_info_file) == FALSE) {
      ...
      maxint <- apply(adjdata2[, -c(1:4)], 1, function(x) {
        max(x, na.rm = TRUE)
      })

      adjdata2 <- cbind(
        adjdata2[, c(1:4)], numpeaks, feat.eval.result.mat$numgoodsamples, feat.eval.result.mat$median, feat.eval.result.mat$Qscore, maxint,
        adjdata2[, -c(1:4)]
      )
  ...
  return(list(
    "apLCMS.merged.res" = union_list, "apLCMS.ind.res" = data_rpd_all, "apLCMS.ind.res.filtered" = data_rpd, "final.feat.table.annot" = annotres_list, "feat.eval.ind" = feateval_list, "sample.eval.ind" = rsqres_list, "final.feat.table.raw" = d1, "final.feat.table.combat" = adjdata2,
